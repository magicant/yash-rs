...
impl OpenFileDescription {
    ...
    /// signal interrupts the operation. `get_waker` has the same contract as
    /// for [`poll_write`](Self::poll_write).
    ///
    /// [`PIPE_BUF`]: super::PIPE_BUF
    pub(super) fn poll_write_full<F>(
        &mut self,
        buffer: &[u8],
        ...
        F: FnMut() -> Weak<Cell<Option<Waker>>>,
    {
        ...
        let loop_on_success = !self.is_nonblocking && self.file.borrow().body.is_fifo();
        loop {
...
mod tests {
    ...
    fn poll_write_full_blocking_fifo_loops_until_full() {
        ...
        // across PIPE_BUF-sized chunks until either the buffer is exhausted
        // or no more bytes fit (returning Pending).
        let fifo = Rc::new(RefCell::new(Inode {
            body: FileBody::Fifo {
                content: VecDeque::new(),
                readers: 1,
        ...
        // much as fits (PIPE_SIZE bytes via multiple chunked poll_write calls)
        // and then returns Pending because the pipe is full.
        let buffer = [7u8; PIPE_SIZE + 1];
        let waker_holder: Rc<Cell<Option<Waker>>> = Rc::new(Cell::new(Some(noop_waker())));
        let mut bytes_written = 0usize;
        let result =
