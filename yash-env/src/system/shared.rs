...
mod tests {
    ...
    fn shared_system_wait_for_signals() {
        ...
        let state = Rc::clone(&system.state);
        let system = SharedSystem::new(system);
        system.set_disposition(SIGCHLD, Disposition::Catch).now_or_never().unwrap().unwrap();
        system.set_disposition(SIGINT, Disposition::Catch).now_or_never().unwrap().unwrap();
        system.set_disposition(SIGUSR1, Disposition::Catch).now_or_never().unwrap().unwrap();
    ...
    fn shared_system_wait_for_signal_ignores_irrelevant_signals() {
        ...
        let system = SharedSystem::new(system);
        system.set_disposition(SIGINT, Disposition::Catch).now_or_never().unwrap().unwrap();
        system.set_disposition(SIGTERM, Disposition::Catch).now_or_never().unwrap().unwrap();
