...
mod tests {
    ...
    fn write_returns_eintr_when_space_and_signal_arrive_simultaneously() {
        // Verifies that EINTR is returned even when space becomes available in the
        // pipe and a signal is caught at the same time (before the next poll).
        // The signal must take precedence over the newly available space.
        let system = VirtualSystem::new();
        system.sigaction(SIGINT, Disposition::Catch).unwrap();
        let (reader, writer) = system.pipe().unwrap();
