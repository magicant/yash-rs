...
- With `portable` enabled, the `ulimit` built-in now rejects the option syntax POSIX does not guarantee: option letters grouped in a single argument (for example, `ulimit -fH`), which POSIX exempts `ulimit` from having to recognize; `-H` and `-S` given together, whether setting or displaying a limit, since POSIX writes the synopsis as `ulimit [-H|-S] ...`; and an option other than `-H` and `-S` specified more than once (for example, `ulimit -f -f`), whose behavior POSIX leaves unspecified. Repeating `-H` or `-S` remains accepted.
