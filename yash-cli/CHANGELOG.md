...
- The `umask` built-in now accepts `--symbolic` as the long name of the `-S` option, and the `unalias` built-in now accepts `--all` as the long name of the `-a` option. The manual described these long names, but the built-ins rejected them as unknown options.
...
- With `portable` enabled, `cd -` now fails if `OLDPWD` is not an absolute path.
...
- With `portable` enabled, the `jobs` built-in now rejects an operand that does not start with a `%` (for example, `jobs 1`) as an error.
...
- With `portable` enabled, built-ins and the shell command line now reject option syntax POSIX does not guarantee — an option a built-in does not require, a long option, or an option argument attached to the option name (for example, `set -oerrexit` or `read -d:`) — as an error, instead of accepting it as an extension. This affects most built-ins, including `cd`, `command`, `exit`, `export`, `jobs`, `pwd`, `read`, `readonly`, `return`, `set`, `trap`, `ulimit`, and `unset`, as well as the options `set` and the shell command line accept. See [Writing portable scripts](https://magicant.github.io/yash-rs/posix.html#writing-portable-scripts) in the manual for what each one rejects.
- With `portable` enabled, the `ulimit` built-in also rejects option letters grouped in a single argument (for example, `ulimit -fH`), `-H` and `-S` given together, and an option other than `-H` or `-S` specified more than once — syntax POSIX leaves unspecified or exempts `ulimit` from supporting portably.
- With `portable` enabled, the `kill` built-in also rejects signal syntax POSIX does not specify: the `-n` and `-v` options, a signal number other than `0` given to `-s`, and more than one operand or a signal name operand given with `-l`. The `kill -SIGNAL`, `kill -signal_number`, and `kill -stop` forms remain accepted, as POSIX specifies them.
- With `portable` enabled, several built-ins now reject a number of operands POSIX does not allow: `export` and `readonly` given neither operands nor `-p`, or both; `unset` given no operands; `command` and `type` given no operands; `command -v`/`-V` given more than one operand or both options together; and `.` given an operand after the *file* operand.
- With `portable` enabled, the `true` and `false` built-ins now print a warning, instead of silently ignoring it, when given any argument, including a lone `--`. POSIX lists both the OPTIONS and OPERANDS sections of these utilities as "None.". The warning goes to the standard error and does not affect the exit status; the arguments are still ignored.
...
- The output of `set +o` now begins with `set +o portable` and no longer lists the option in its alphabetical position; if the option is on, a final `set -o portable` restores it. The option names in between are yash's own rather than the ones POSIX specifies, so the leading command keeps the output executable regardless of whether the shell running it has `portable` set.
...
[3.4.4]: https://github.com/magicant/yash-rs/releases/tag/yash-cli-3.4.4
