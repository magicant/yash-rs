...
## When traps are executed

Traps are handled at specific points during shell execution:

- Signal traps run when signals are caught, typically before and after command execution.
- `EXIT` traps run when the shell exits normally, after all other commands complete.
- Traps do not interrupt running commands; they wait for commands to finish.
