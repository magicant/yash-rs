...
Before diving into the details, let's get a rough understanding of what job control is all about.
...
The `fg` built-in brings a job to the foreground and sends it a `SIGCONT` signal to resume its execution. The job then continues running as the foreground process group of the terminal, allowing you to interact with it again.

The `bg` built-in sends a `SIGCONT` signal to a job without bringing it to the foreground. It allows the job to continue running in the background, so you can continue using the shell for other tasks.

### Signaling jobs

The `kill` built-in sends a signal to a process or a process group. It accepts job IDs (see below) to specify jobs as target process groups. This allows low-level control over jobs, such as terminating or suspending them. For example, you can use `kill -STOP %1` to suspend job number 1, or `kill -KILL %2` to terminate job number 2.
...
The shell notifies the user of job status changes before showing a prompt for the next command. This automatic notification includes jobs whose status has changed since the last notification.

The `jobs` built-in can be used to display the current job list at any time.

The output format of these notifications and the `jobs` built-in includes the job number, status, and command string of each job. If the job has been suspended, the status includes the signal that suspended it, such as `SIGTSTP`. If the job has been terminated, the status includes the exit status or signal that terminated it.

## Additional details

The following subsections address special cases and additional features of job control you would not normally care about in everyday use.
