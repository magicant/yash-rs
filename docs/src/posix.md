...
- (Since 3.3.3) An operand of the [`export` built-in](builtins/export.md), the [`readonly` built-in](builtins/readonly.md), the [`typeset` built-in](builtins/typeset.md), or the [`read` built-in](builtins/read.md) specifying a [variable name](language/parameters/variables.md#variable-names) that contains a character other than underscores, digits, and alphabetics from the portable character set, or that starts with a digit.
