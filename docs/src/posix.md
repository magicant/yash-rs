...
- (Since 3.5.0) Both `-` and `--` given as option-operand separators on the [command line](startup.md#compatibility).
...
- (Since 3.5.0) A `-` used as an option-operand separator with the [`set` built-in](builtins/set.md) (for example, `set - foo`).
