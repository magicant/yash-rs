...
(Since 3.4.1) When the [`portable` option](../environment/options.md#portable) is set, the built-in rejects a `-` that would act as an option-operand separator with an error. A `-` that is not a separator, as in `set -- -`, remains an ordinary operand.
