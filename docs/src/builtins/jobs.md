...
A portable job ID must start with a `%`. If an operand does not have a leading `%`, the built-in assumes one silently, which is not portable. (Since 3.4.1) When the [`portable` option](../environment/options.md#portable) is set, the built-in rejects an operand without the leading `%`, as well as the lone `%`, which POSIX does not specify as a [job ID](../interactive/job_control.md#job-ids) either.
