...
- Public dependency versions:
    - yash-env 0.16.0 → 0.17.0
    - yash-syntax 0.24.0 → 0.25.0
...
- Executing an elective or extension built-in in a simple command now fails
  ...
  before the error is reported. This also applies to a special built-in
  invoked under a name POSIX does not define as a special built-in name (for
  example, `source`, a non-standard alias for the `.` built-in); `.` itself is
  unaffected.
- Public dependency versions:
...
- The read-eval loop now sets the lexer's parsing mode
  (`yash_syntax::parser::lex::Lexer::set_mode`) from the current shell options
  before parsing each command line, so that options affecting the accepted
...
- Public dependency versions:
    - yash-arith 0.2.1 → 0.2.3
    - yash-env 0.12.0 → 0.13.0
    - yash-syntax 0.19.0 → 0.20.0
...
[0.21.1]: https://github.com/magicant/yash-rs/releases/tag/yash-semantics-0.21.1
[0.21.0]: https://github.com/magicant/yash-rs/releases/tag/yash-semantics-0.21.0
[0.20.0]: https://github.com/magicant/yash-rs/releases/tag/yash-semantics-0.20.0
