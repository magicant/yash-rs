...
- `umask::syntax::parse` now accepts `--symbolic` as the long name of the `-S`
  option, and `unalias::syntax::parse` now accepts `--all` as the long name of
  the `-a` option. The long names were documented but not implemented.
  Accordingly, the report of `unalias::syntax::Error::ConflictingOptionAndOperand`
  now names the option `-a/--all`.
...
[0.12.0]: https://github.com/magicant/yash-rs/releases/tag/yash-builtin-0.12.0
