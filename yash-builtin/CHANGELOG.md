...
- The `set` built-in (`set::syntax::parse`) now rejects a `-` used as a
  ...
  on. POSIX leaves the results unspecified if the first argument is `-`, and
  shells disagree on what such a `-` means. A `-` that is not consumed as a
  separator remains an ordinary operand.
